import React, { useState, useEffect } from 'react';
import YouTube from 'react-youtube';
import FPLyrics from './FPLyrics';
import '../../styles/components/FPVideoPlayer.css';


const FPVideoPlayer = ({
    youtubeData,
    currentTrack,
    showLyrics,
    position,
    handleSeek,
    artistNames,
    duration,
    onVersionChange, // Callback to notify parent about active version index
    onOpenLivePage,
    activeVersionIndex
}) => {
    const [embedError, setEmbedError] = useState(false);
    const [selectedVersionIndex, setSelectedVersionIndex] = useState(0);

    // Reset state when track changes
    useEffect(() => {
        setEmbedError(false);
        setSelectedVersionIndex(0);
        if (onVersionChange) onVersionChange(0);
    }, [youtubeData?.youtubelinkID]);

    // Constructively sync internal state with parent prop if provided
    useEffect(() => {
        if (activeVersionIndex !== undefined && activeVersionIndex !== null && activeVersionIndex !== selectedVersionIndex) {
            setSelectedVersionIndex(activeVersionIndex);
        }
    }, [activeVersionIndex]);

    // Parse IDs
    const rawIds = youtubeData?.youtubelinkID ? youtubeData.youtubelinkID.split(',') : [];
    const hasLiveOption = rawIds.length > 1;

    // Determine active video ID
    const activeIndex = (selectedVersionIndex >= 0 && selectedVersionIndex < rawIds.length) ? selectedVersionIndex : 0;
    const activeVideoId = rawIds[activeIndex] ? rawIds[activeIndex].trim() : "";

    // Notify parent of version change
    useEffect(() => {
        if (onVersionChange) onVersionChange(activeIndex);
    }, [activeIndex]);

    const handleVersionSelect = (index) => {
        setSelectedVersionIndex(index);
    };

    return (
        <div className={`fp-right-section ${showLyrics ? 'lyrics-mode' : ''}`}>
            {/* Live Toggle Button - Positioned absolute relative to fp-right-section */}
            {!showLyrics && hasLiveOption && (
                <button
                    className="fp-live-toggle-btn"
                    onClick={(e) => {
                        e.stopPropagation();
                        if (onOpenLivePage) onOpenLivePage();
                    }}
                    title="View Live Performances"
                >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M23 7l-7 5 7 5V7z"></path>
                        <rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect>
                    </svg>
                    <span>Live</span>
                </button>
            )}

            {showLyrics ? (
                <FPLyrics
                    lyricsFileName={youtubeData.lyrics}
                    position={position}
                    handleSeek={handleSeek}
                    trackName={currentTrack.name}
                    artistNames={artistNames}
                    artistName={currentTrack.artists?.[0]?.name || artistNames}
                    albumName={currentTrack.album?.name}
                    duration={currentTrack.duration_ms ? currentTrack.duration_ms / 1000 : duration}
                />
            ) : (
                <>
                    <div className="fp-youtube-card">

                        {/* Embed Mode with Fallback */}
                        {youtubeData?.useEmbed && !embedError ? (
                            <YouTube
                                videoId={activeVideoId}
                                className="fp-youtube-player-wrapper"
                                iframeClassName="fp-youtube-embed"
                                opts={{
                                    width: '100%',
                                    height: '100%',
                                    playerVars: {
                                        autoplay: 0,
                                        rel: 0,
                                        modestbranding: 1
                                    },
                                }}
                                onError={() => setEmbedError(true)}
                                style={{ width: '100%', height: '100%', borderRadius: '12px', overflow: 'hidden' }}
                            />
                        ) : (
                            <>
                                <img
                                    src={`https://img.youtube.com/vi/${activeVideoId}/maxresdefault.jpg`}
                                    alt="YouTube Thumbnail"
                                    className="fp-youtube-thumb"
                                    onError={(e) => {
                                        e.target.onerror = null;
                                        e.target.src = `https://img.youtube.com/vi/${activeVideoId}/hqdefault.jpg`;
                                    }}
                                    onLoad={(e) => {
                                        if (e.target.src.includes('maxresdefault.jpg') && e.target.naturalWidth === 120) {
                                            e.target.src = `https://img.youtube.com/vi/${activeVideoId}/hqdefault.jpg`;
                                        }
                                    }}
                                />
                                <div className="fp-youtube-overlay">
                                    <div className="fp-overlay-buttons">
                                        <button className="fp-watch-btn transparent-play" onClick={() => {
                                            let qualityParam = "";
                                            if (youtubeData.format) {
                                                switch (youtubeData.format) {
                                                    case "4320p": qualityParam = "&vq=hd4320"; break;
                                                    case "2160p": qualityParam = "&vq=hd2160"; break;
                                                    case "1440p": qualityParam = "&vq=hd1440"; break;
                                                    case "1080p": qualityParam = "&vq=hd1080"; break;
                                                    case "720p": qualityParam = "&vq=hd720"; break;
                                                    case "480p": qualityParam = "&vq=large"; break;
                                                    case "360p": qualityParam = "&vq=medium"; break;
                                                    case "240p": qualityParam = "&vq=small"; break;
                                                    case "144p": qualityParam = "&vq=tiny"; break;
                                                    default: qualityParam = "";
                                                }
                                            }
                                            window.open(`https://www.youtube.com/watch?v=${activeVideoId}${qualityParam}`, '_blank');
                                        }}>
                                            <svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M8 5v14l11-7z" />
                                            </svg>
                                        </button>
                                    </div>
                                </div>
                            </>
                        )}
                    </div>

                    <div className="fp-youtube-info">
                        <div>
                            <h3>Available on YouTube</h3>
                            <p>{youtubeData.name} • {youtubeData.quality || youtubeData.format}</p>
                        </div>
                    </div>
                </>
            )}
        </div>
    );
};

export default FPVideoPlayer;
