 ...
 * Run: node scripts/dedup-actors.cjs
 */
...
if (data.categories) {
  data.categories.forEach((cat) => {
    ...
    if (removed > 0) {
      console.log(
        `  "${cat.name}": removed ${removed} duplicate(s) (${before} → ${cat.actors.length})`,
      );
    } else {
      console.log(
        `  "${cat.name}": no duplicates (${cat.actors.length} actors)`,
      );
    }
  ...
  console.log(
    `  Removed ${removed} duplicate(s) (${before} → ${data.actors.length})`,
  );
  totalRemoved = removed;
}
